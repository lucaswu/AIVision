...
def get_hr_config():
    ...
    # 解析相对路径：当基于外部项目的 cwd 运行时，相对路径会失效。
    # 强制让其相对于 IQIDDET 根目录来解析
    from pathlib import Path
    yaml_path = Path(hr_yaml)
    if not yaml_path.is_absolute():
        iqiddet_root = Path(__file__).resolve().parents[2]
        yaml_path = iqiddet_root / yaml_path
        
    if not yaml_path.is_file():
        raise ValueError(f"hrnet config not found: {yaml_path}")

    cfg.update(cfg.from_yaml(filename=str(yaml_path)))
